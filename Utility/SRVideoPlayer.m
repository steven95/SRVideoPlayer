...
- (UIButton *)replayBtn {
    ...
    if (!_replayBtn) {
        ...
        NSString* path = [NSString stringWithFormat:@"SRVideoPlayer.bundle/%@",@"replay@2x"];
        NSString *file1 = [bundle pathForResource:path ofType:@"png"];
        UIImage *image1 = [UIImage imageWithContentsOfFile:file1];
        [_replayBtn setImage:image1 forState:UIControlStateNormal];
...
- (instancetype)initWithVideoURL:(NSURL *)videoURL playerView:(UIView *)playerView playerSuperView:(UIView *)playerSuperView {
    ...
    if (self = [super init]) {
        ...
        self.playerLayer =  (AVPlayerLayer *)self.videoLayerView.layer;
...
- (void)appWillResignActive {
    ...
    NSBundle* bundle = [NSBundle bundleForClass:[self class]];
    NSString* path = [NSString stringWithFormat:@"SRVideoPlayer.bundle/%@",@"start@2x"];
    NSString *file1 = [bundle pathForResource:path ofType:@"png"];
    UIImage *image1 = [UIImage imageWithContentsOfFile:file1];
    [self.bottomBar.playPauseBtn setImage:image1 forState:UIControlStateNormal];
...
- (void)appDidBecomeActive {
    ...
    if (!_isManualPaused) {
        NSBundle* bundle = [NSBundle bundleForClass:[self class]];
        NSString* path = [NSString stringWithFormat:@"SRVideoPlayer.bundle/%@",@"pause@2x"];
        NSString *file1 = [bundle pathForResource:path ofType:@"png"];
        UIImage *image1 = [UIImage imageWithContentsOfFile:file1];
        [self.bottomBar.playPauseBtn setImage:image1 forState:UIControlStateNormal];
...
- (void)playerItemDidPlayToEnd:(NSNotification *)notification {
    ...
    switch (_playerEndAction) {
        case SRVideoPlayerEndActionStop:
            ...
            if (self.playStatueBlock) {
                self.playStatueBlock(3);
            }
            break;
...
- (void)play {
    ...
    [_player addPeriodicTimeObserverForInterval:CMTimeMake(1.0, 1.0) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        CGFloat timeDuration = CMTimeGetSeconds(time);
        if (weakSelf.playSecondsBlock) {
            weakSelf.playSecondsBlock(timeDuration);
        }
    }];
...
- (void)changeToOrientation:(UIInterfaceOrientation)orientation {
    ...
    if (orientation == UIInterfaceOrientationPortrait || orientation == UIInterfaceOrientationPortraitUpsideDown) {
        ...
        NSBundle* bundle = [NSBundle bundleForClass:[self class]];
        NSString* path = [NSString stringWithFormat:@"SRVideoPlayer.bundle/%@",@"full_screen@2x"];
        NSString *file1 = [bundle pathForResource:path ofType:@"png"];
        UIImage *image1 = [UIImage imageWithContentsOfFile:file1];
        [_bottomBar.changeScreenBtn setImage:image1 forState:UIControlStateNormal];
    }
    ...
    if (orientation == UIInterfaceOrientationLandscapeLeft || orientation == UIInterfaceOrientationLandscapeRight) {
        ...
        NSBundle* bundle = [NSBundle bundleForClass:[self class]];
        NSString* path = [NSString stringWithFormat:@"SRVideoPlayer.bundle/%@",@"small_screen@2x"];
        NSString *file1 = [bundle pathForResource:path ofType:@"png"];
        UIImage *image1 = [UIImage imageWithContentsOfFile:file1];
        [_bottomBar.changeScreenBtn setImage:image1 forState:UIControlStateNormal];
    }
    ...
    if (pan.state == UIGestureRecognizerStateChanged) {
        ...
        if (_controlType == SRControlTypeProgress) {
            ...
            if (videoCurrentTime > _touchBeginVideoValue) {
                NSBundle* bundle = [NSBundle bundleForClass:[self class]];
                NSString* path = [NSString stringWithFormat:@"SRVideoPlayer.bundle/%@",@"progress_right@2x"];
                NSString *file1 = [bundle pathForResource:path ofType:@"png"];
                UIImage *image1 = [UIImage imageWithContentsOfFile:file1];
                [self.videoOperationTip setTipImageViewImage:image1];
...
- (void)videoBottomBarDidClickPlayPauseBtn {
    ...
    switch (_playerState) {
        case SRVideoPlayerStatePlaying:
            [self pause];
            if (self.playStatueBlock) {
                self.playStatueBlock(2);
            }
            break;
        case SRVideoPlayerStatePaused:
            [self resume];
            if (self.playStatueBlock) {
                self.playStatueBlock(1);
            }
            break;
        default:
            break;
...
- (void)videoBottomBarDidEndChangeSlider:(UISlider *)slider {
    ...
    self.bottomBar.currentTimeLabel.text = [self formatTimeWith:(long)ceil(slider.value)];
    NSBundle* bundle  =   [NSBundle bundleWithPath:[[NSBundle bundleForClass:[self class]] pathForResource:@"SRVideoPlayer" ofType:@"bundle"]];
    UIImage *image1 = [UIImage imageWithContentsOfFile:[bundle pathForResource:@"pause@2x" ofType:@"png"]];
    [self.bottomBar.playPauseBtn setImage: image1  forState:UIControlStateNormal];
